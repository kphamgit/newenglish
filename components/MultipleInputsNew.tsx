...
interface MultipleInputsProps {
 ...
 const MultipleInputsNew: React.FC<MultipleInputsProps> = ({ ref, content, enableCheckButton }) => {
 ...
 useImperativeHandle(ref, () => ({
    ...
    if (item.type === 'text') {
      ...
      return (
        <TextInput
          ...
          style={[{width: longestBlankWidth.current}, styles.input]}
  ...
  const computedBlankLayouts = inputFields?.map((item, index) => {
   ...
   return (
    ...
    <View>
      {blankWidthsRef.current.length > 0 && (
        <Text style={{color: 'black'}}>
          Debug: Blank widths: {blankWidthsRef.current.map((width, idx) => `Blank ${idx}: ${width.toFixed(2)}px`).join(', ')}
        </Text>
      )}
    </View>
    </>
...
export default MultipleInputsNew;
